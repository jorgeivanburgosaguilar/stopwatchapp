...
| S17 Manage Records window | ✅ Done | 2026-09-13 | Enabled the existing Manage Records action and added a single modeless manager window with all records newest-first, 10-record pagination, confirmed per-row delete, and a confirmed header Clear All action. Added delete-by-id persistence and timer reload/event coverage without a schema migration; `<Version>` bumped to `1.4.0`. |
...
- **Files:** `Theme/Typography.cs` (new); `MainForm.cs`, `Controls/StopwatchControl.cs`,
  ...
  `<Version>` → `1.1.0`); `Assets/NOTICE.md` records the Fluent System Icons attribution and MIT
  license for the committed icon. The one-off generator originally used for the icon was later
  removed because it is not needed by the project.
