...
`GetBounds()` — fits its budget: the 32×32 canvas for the three single-label layouts, each half's own
box for the diagonal one. Measure ink, not `MeasureString` output: `MeasureString`
...
- **The tray icon remains a square notification icon.** Windows gives third-party notification-area
  ...
  large minutes, hours, days, and each half of the hour/minute split, while the tooltip carries
  exact unbounded time. The 1:00–9:59 band is a corner-anchored diagonal (hour top-left, minutes
  bottom-right, thin slash between) rather than a single four-glyph `h:mm` line: one horizontal line
  is width-bound and left the top and bottom of the square slot empty, while anchoring to opposite
  corners makes each half height-bound and much larger. The two boxes are disjoint across the
  anti-diagonal so no collision handling is needed at draw time, and the slash occupies only the
  corridor the anchoring already leaves free. The 10H–23H and day labels stay single-label, since
  two or three glyphs already fill the canvas.
