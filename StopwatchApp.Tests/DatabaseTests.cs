...
public sealed class DatabaseTests : IAsyncLifetime
{
  ...
  {
    await _database.SaveRecordAsync(
      startTimestamp: 0,
      endTimestamp: 30_000,
      elapsedMs: 30_000,
      laps: []
  ...
  [Fact]
  public async Task SaveRecordAsync_WithLaps_RoundTripsThemNewestFirst()
  {
    Lap[] laps =
    [
      new Lap(1, 0, 60_000, 1),
      new Lap(2, 60_000, 125_000, 1),
      new Lap(3, 125_000, 190_000, 1),
    ];
    ...
    Assert.Equal(laps[0], loadedLaps[2]);
  }

  [Fact]
  public async Task SaveRecordAsync_WithNoLaps_ReturnsAnEmptyLapsList()
  {
  ...
  [Fact]
  public async Task GetAllRecordsAsync_ReportsLapCount()
  {
    ...
    Assert.Equal(1, records[1].LapCount);
  }

  [Fact]
  public async Task DeleteRecordAsync_RemovesOnlyThatRecordsLaps()
  {
    long keptId = await _database.SaveRecordAsync(0, 60_000, 60_000, [new Lap(1, 0, 60_000, 1)]);
    long deletedId = await _database.SaveRecordAsync(1, 60_000, 60_000, [new Lap(1, 0, 60_000, 1)]);
  ...
  [Fact]
  public async Task ClearAllRecordsAsync_RemovesEveryRecordsLaps()
  {
    long firstId = await _database.SaveRecordAsync(0, 60_000, 60_000, [new Lap(1, 0, 60_000, 1)]);
    long secondId = await _database.SaveRecordAsync(1, 60_000, 60_000, [new Lap(1, 0, 60_000, 1)]);
